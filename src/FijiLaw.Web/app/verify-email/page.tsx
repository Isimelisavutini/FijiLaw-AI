...
export default function VerifyEmailPage(){
  ...
  async function verifyToken(value:string){
    ...
    try{
      const response=await fetchWithTimeout(`${API_BASE}/api/auth/verify-email`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:value.trim()})},12000);
  ...
  async function verify(e:FormEvent){e.preventDefault();await verifyToken(token);}

  const actionDisabled=loading||!token.trim()||success;
  ...
  return <main style={{maxWidth:720,margin:'0 auto',padding:'64px 24px',fontFamily:'Inter,system-ui,sans-serif',color:'#16231c'}}>
    ...
    <h1 style={{fontFamily:'Georgia,serif',fontWeight:500,fontSize:52,margin:'8px 0 16px'}}>{loading?'Verifying your email…':success?'Email verified.':'Verify your FijiLaw account.'}</h1>
    <p style={{color:'#5c6b62',lineHeight:1.7}}>Verification is required before paid dashboard features are enabled. Links from FijiLaw verification emails are processed automatically.</p>
    <form onSubmit={verify} style={{background:'#fff',border:'1px solid #d5ddd7',borderRadius:18,padding:28,marginTop:28}}>
      ...
      <input required value={token} onChange={e=>setToken(e.target.value)} placeholder="Paste verification token" autoComplete="one-time-code" disabled={loading||success} style={{width:'100%',padding:'13px 14px',border:'1px solid #bdc8c0',borderRadius:10,fontSize:16,boxSizing:'border-box',marginBottom:14}}/>
      ...
      <button disabled={actionDisabled} style={{width:'100%',border:0,borderRadius:10,padding:14,background:'#173f2b',color:'#fff',fontWeight:800,cursor:actionDisabled?'not-allowed':'pointer',opacity:actionDisabled?0.58:1}}>{loading?'Verifying…':success?'Verified':'Verify email'}</button>
    </form>
